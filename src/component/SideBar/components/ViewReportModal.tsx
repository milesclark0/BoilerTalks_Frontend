...
type ViewReportProps = {
  ...
  setReportsList: React.Dispatch<
    React.SetStateAction<
      { id: string; timeSent: string; username: string; reason: string; body: string; numBans: number; numWarns: number; recipient?: string }[]
    >
  >;
      ...
      <Box component={"form"} sx={{}}>
        ...
        <DialogContent>
          ...
          >
            <Box>
              Sort By:{" "}
              <Button variant="outlined" onClick={handleClickSort}>
                {reportStateText}
              </Button>
            </Box>
              ...
              <TableBody>
                {ReportsList.map((report) => (
                  <TableRow key={report.id} sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
                    ...
                    <IconButton onClick={() => handleRemoveReport(report["id"])}>
                      <DeleteIcon />
                    </IconButton>
                  </TableRow>
                ))}
